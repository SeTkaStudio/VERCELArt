...
            if (adminUser) {
                // АДМИНИСТРАТОР: Устанавливаем credits для тестирования списаний.
                const userToSet = isAppView 
                  ? { ...adminUser, username: 'Admin', paymentMethod: 'credits' } 
                  : adminUser;
...
            if (user) {
                // ОБЫЧНЫЙ ПОЛЬЗОВАТЕЛЬ: Автоматически устанавливаем 'apiKey' для использования общего ключа.
                const userWithDefaultMethod = { ...user, paymentMethod: 'apiKey' };
                setCurrentUser(userWithDefaultMethod);
            } else {
                setCurrentUser(null);
            }
        } else {
...
    if (user) {
      // При входе явно устанавливаем метод оплаты 'apiKey', чтобы использовать общий ключ.
      const userWithDefaultMethod = { ...user, paymentMethod: 'apiKey' as 'credits' | 'apiKey' };
      setCurrentUser(userWithDefaultMethod);
