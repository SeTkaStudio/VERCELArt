...
// Мы добавим ее в конец файла, как и раньше, но сначала исправим старые функции.
...
// =====================================================================
// ИСПРАВЛЕННЫЕ ФУНКЦИИ GOOGLE: УДАЛЕНА ПРОВЕРКА process.env.API_KEY
// =====================================================================
...
export const generatePortrait = async (
    ...
    // ИСПРАВЛЕНИЕ: Удалена строка 'const effectiveApiKey = apiKey || process.env.API_KEY;'
    // Теперь ключ должен быть передан через аргумент 'apiKey' (ключ пользователя в режиме "credits")
    const effectiveApiKey = apiKey; 

    if (!effectiveApiKey) {
        throw new Error("API_KEY is not available for this operation."); // Более корректное сообщение
    }
...
export const generateImageWithGemini = async (prompt: string, aspectRatio: AspectRatio, resolution: string, formatFile: ImageFile, styleFile: ImageFile | null, apiKey?: string): Promise<string> => {
    // ИСПРАВЛЕНИЕ: Удалена строка 'const effectiveApiKey = apiKey || process.env.API_KEY;'
    const effectiveApiKey = apiKey;
    if (!effectiveApiKey) throw new Error("API_KEY is not available for this operation.");
    const ai = new GoogleGenAI({ apiKey: effectiveApiKey });
