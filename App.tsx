...
const App: React.FC<AppProps> = ({ onNavigateHome, images, setImages }) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    for (const variation of variations) {
      ...
      try {
        ...
        const resultSrc = await generatePortrait(imageFile, fullPrompt, backgroundFile, clothingFile, apiKey);
        if (isGenerationCancelled.current) break;
  ...
  const handleRegenerate = useCallback(async (imageId: string) => {
    ...
    try {
        const resultSrc = await generatePortrait(imageFile, imageToRegen.prompt, backgroundFile, clothingFile, apiKey);
